...
【版本说明 303】
  原始文件：大创agentV3/core/music_analyzer.py  （内容完整保留，未改动）
  本次新增：麦克风诊断功能
    - DEBUG_DUMP_AUDIO  : 将采集到的原始 PCM 保存为 WAV 文件，用于验证咪头是否工作
    - DEBUG_LOG_ENERGY  : 在音频回调中每隔 N 帧打印一次瞬时能量，便于实时观察
  以上调试功能通过顶部两个开关控制，测试完毕后设为 False 即可静默。
"""

# ==============================================================
# 调试开关（测试完毕后改为 False 即可关闭，不影响其他逻辑）
# ==============================================================
DEBUG_DUMP_AUDIO = False          # True → 将采集音频保存为 WAV 文件
DEBUG_DUMP_PATH  = "/tmp/mic_debug_{ts}.wav"   # WAV 输出路径；{ts} 自动替换为时间戳
DEBUG_DUMP_SECONDS = 8.0         # 保存前几秒的音频（达到后自动写文件，仅写一次）

DEBUG_LOG_ENERGY = True          # True → 在回调中每 N 帧打印原始能量
DEBUG_LOG_EVERY_N = 43           # 每 N 个 chunk 打印一次（~1s @ 22050Hz/1024）
# ==============================================================

# 麦克风增益系数（应用后将进行削波保护）
# 1.0 = 不增益；2.0~5.0 适合弱信号和远距离麦克风；过大可能引入失真
AUDIO_GAIN_FACTOR = 3.0
...
class MusicAnalyzer:
    ...
    def __init__(
        ...
        # ---- 诊断状态（DEBUG_DUMP_AUDIO / DEBUG_LOG_ENERGY 专用）----
        self._dbg_chunk_count: int = 0          # 回调已处理的帧数
        self._dbg_raw_frames: List[np.ndarray] = []  # 用于 WAV 存储的原始帧
        self._dbg_frames_samples: int = 0       # 已积累的采样点数
        self._dbg_dump_done: bool = False        # 是否已完成 WAV 写入
        # ---------------------------------------------------------------
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"[MicCheck] 默认输入设备: {info.get('name')} "
                    f"| index={info.get('index')} "
                    f"| maxInputChannels={info.get('maxInputChannels')} "
                    f"| defaultSampleRate={info.get('defaultSampleRate')}"
                )
            except Exception as _e:
                print(f"[MicCheck] ⚠️ 无法获取输入设备信息: {_e}")
            # ------------------------------------

            # 【修复4】重置上轮遗留特征，防止 get_current_features() 返回旧数据
            self.current_features = MusicFeatures()
            self.features_history = []

            # 重置诊断状态
            self._dbg_chunk_count = 0
            self._dbg_raw_frames = []
            self._dbg_frames_samples = 0
    ...
    def stop(self) -> None:
        ...
        self.buffer_duration = 0.0  # 【修复3】重置缓冲时长，防止重启时跳过第一个分析窗口
        logger.info("音乐分析已停止")
    ...
    def _audio_callback(self, in_data, frame_count, time_info, status):
        ...
        if self.is_analyzing:
            try:
                ...
                # 【增益】放大信号，使 RMS 进入可识别范围； np.clip 防止溢出
                if AUDIO_GAIN_FACTOR != 1.0:
                    audio_data = np.clip(audio_data * AUDIO_GAIN_FACTOR, -1.0, 1.0)

                # ---- 诊断：原始能量打印（DEBUG_LOG_ENERGY）----
                if DEBUG_LOG_ENERGY:
                    self._dbg_chunk_count += 1
                    if self._dbg_chunk_count % DEBUG_LOG_EVERY_N == 0:
                        raw_e = float(np.mean(audio_data ** 2))
                        raw_peak = float(np.max(np.abs(audio_data)))
                        print(
                            f"[MicCheck] chunk#{self._dbg_chunk_count:05d} "
                            f"raw_energy={raw_e:.8f}  peak={raw_peak:.6f}"
                        )
                # -----------------------------------------------

                # ---- 诊断：WAV 积累（DEBUG_DUMP_AUDIO）----
                if DEBUG_DUMP_AUDIO and not self._dbg_dump_done:
                    self._dbg_raw_frames.append(audio_data.copy())
                    self._dbg_frames_samples += len(audio_data)
                    if self._dbg_frames_samples >= int(DEBUG_DUMP_SECONDS * self.sample_rate):
    ...
    def _save_debug_wav(self) -> None:
        """
        将 _dbg_raw_frames 中的 Float32 PCM 数据保存为 16-bit PCM WAV 文件。
        文件路径由 DEBUG_DUMP_PATH 决定（{ts} 替换为时间戳）。
        """
        try:
            ts = time.strftime("%H%M%S")
            out_path = DEBUG_DUMP_PATH.replace("{ts}", ts)
            ...
            # 合并所有帧并转换为 int16（Float32 范围 -1~1 → int16 -32768~32767）
            merged = np.concatenate(self._dbg_raw_frames).astype(np.float32)
            # 削波保护
            merged = np.clip(merged, -1.0, 1.0)
            pcm16 = (merged * 32767).astype(np.int16)
            ...
            duration_s = len(pcm16) / self.sample_rate
            max_amp = float(np.max(np.abs(merged)))
            mean_e  = float(np.mean(merged ** 2))
            print(
                f"\n[MicCheck] ✅ WAV 已保存: {out_path}\n"
                f"           时长={duration_s:.1f}s | 峰值幅度={max_amp:.6f} | 均方能量={mean_e:.8f}\n"
                f"           → 用 aplay {out_path} 或播放器收听，确认咪头是否正常采音\n"
            )
            logger.info(f"[MicCheck] 诊断 WAV 已保存: {out_path}  max_amp={max_amp:.6f}")

        except Exception as exc:
            print(f"[MicCheck] ⚠️ WAV 保存失败: {exc}")
            logger.warning(f"[MicCheck] WAV 保存失败: {exc}")
    # ------------------------------------
