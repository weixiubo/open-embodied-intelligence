...
【语音优化】支持"跳x秒"命令格式，避免"舞"同音词干扰，支持汉字数字识别
【303修正】兼容"跳"字的 ASR 误识别形式（"1"同音调 / "要"近音），联合"秒"触发
"""
...
from utils.helpers import extract_duration_from_text  # 【修改1】新增导入：支持汉字数字解析
...
class RobotController:
    ...
    def _is_music_ready(self, features: MusicFeatures, wait_start_time: float) -> tuple:
        ...
        min_analysis_duration = 10.0  # 【增益计时】自首帧有效音乐起必须达到 10s 才允许就绪
    ...
    def _wait_for_music_ready(self, timeout_seconds: float = 6.0) -> bool:
        ...
        music_start_time: Optional[float] = None  # 【计时起点】首帧有效音乐的时间戳

        while not self.stop_event.is_set() and time.time() < deadline:
            features = self.music_analyzer.get_current_features()

            # 【计时起点】首次检测到有效能量时记录起点
            if music_start_time is None and features.energy > 0.00001:
                music_start_time = time.time()
                print(f"\n[音乐检测] 首次检测到有效音频 (energy={features.energy:.6f})，10s 分析计时开始\n")
                logger.info(f"【计时起点】首帧有效音乐 energy={features.energy:.6f}")

            # 将 music_start_time 作为 _is_music_ready 的计时参考点；
            # 未检测到音乐时传入 deadline+1 保证 analyzed=False
            timing_ref = music_start_time if music_start_time is not None else (deadline + 1)
            is_ready, diagnostics = self._is_music_ready(features, timing_ref)
            check_count += 1

            elapsed      = time.time() - wait_start_time          # 总等待时间（用于 deadline 判断）
            music_elapsed = (time.time() - music_start_time) if music_start_time is not None else 0.0  # 音乐计时
            current_time = time.time()

            # 单行进度条显示（每 0.5s 更新一次）——基于音乐计时
            if current_time - last_progress_time >= 0.5:
                import sys
                progress_pct = min(music_elapsed / min_window, 1.0)
                bar_len = 20
                filled = int(bar_len * progress_pct)
                bar = '█' * filled + '░' * (bar_len - filled)
                sys.stdout.write(f"\r 音乐分析中 [{bar}] {music_elapsed:.1f} / {min_window:.1f}s | beats={len(features.beat_times)} | energy={features.energy:.5f}")
                sys.stdout.flush()
                last_progress_time = current_time

            logger.debug(f"【等待音乐】检查#{check_count} @ wall={elapsed:.1f}s music={music_elapsed:.1f}s: {diagnostics}")
    ...
    def handle_voice_command(self, text: str) -> bool:
        ...
        【语音优化】改为"跳x秒"格式，支持汉字数字（一、二、十、二十五等）
        ...
        Returns:
            是否处理了命令
        """
        # 【修改2】跳舞命令 - 使用 extract_duration_from_text() 支持汉字数字
        # 原因：避免"舞"与"5"同音导致误识别，且用户常说"跳五秒"而非"跳5秒"
        #
        # 【修改5】兼容"跳"字的常见 ASR 误识别形式
        #   - "跳" → "1"（同音调）: 例如 "1二十秒"
        #   - "跳" → "要"（近音） : 例如 "要二十秒"
        #   联合"秒"做二次确认，避免日常短句中单独出现"1"/"要"被误触发
        _has_jump   = "跳" in text
        _has_dance  = "舞蹈" in text                                          # 【修改6】新增"舞蹈X秒"触发词
        _has_alias  = any(w in text for w in ("1", "要")) and "秒" in text
        if _has_jump or _has_dance or _has_alias:
            duration = extract_duration_from_text(text)  # 自动支持：跳五秒、跳20秒、跳二十秒

            # 【修改6】"舞蹈"命令无数字时使用默认时长 5 秒
            if duration is None and _has_dance:
                duration = 5
            ...
            if duration:
                ...
                # 【修改3】错误提示：明确告诉用户支持中文数字
                logger.warning(
                    f"❌ 【语音解析】无法解析有效时长: "
                    f"原文=\"{text}\", "
                    f"请说：跳[5-60]秒 或 舞蹈[5-60]秒 (支持中文：跳十秒、舞蹈二十秒)"
                )
                return False
