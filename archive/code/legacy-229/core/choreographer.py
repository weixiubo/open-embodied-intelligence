...
class Choreographer:
    ...
    def select_action(
        ...
        logger.debug(f"【编舞器】进入动作选择: 可用={len(available_actions)}个, 剩余时间={remaining_time_ms/1000:.1f}s")
        ...
        # 【问题1】输出前3候选动作及得分
        logger.debug(f"【编舞器】候选动作TOP3:")
        for i, (act, sc) in enumerate(scores[:3]):
            logger.debug(f"  {i+1}. {act.label} (得分={sc:.3f})")
        ...
        if selected:
            ...
            logger.debug(f"【编舞器】最终选择: {selected.label}, 理由: {reason}")
