        ...
        while self.is_analyzing and not self.stop_event.is_set():
            try:
                if not self.audio_buffer.empty():
                    ...
                    last_buffer_check = time.time()  # 更新时间戳
                else:
                    # 检查是否超时
                    if time.time() - last_buffer_check > analysis_timeout:
                        logger.warning(f"音乐分析超时（{analysis_timeout}秒无数据），可能音频流异常")
                        last_buffer_check = time.time()  # 重置时间戳继续监控
                    time.sleep(0.05)
