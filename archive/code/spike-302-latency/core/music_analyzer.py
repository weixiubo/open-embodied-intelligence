...
【增强版】增加详细的阶段性调试输出。
【优化版】降低噪声阈值，缩短分析窗口，改进beat_track检测。
"""
...
class MusicAnalyzer:
    ...
    def __init__(
        ...
        # 【问题3】音频质量监控 - 【修改1】降低噪声阈值
        self.noise_threshold = 250  # 【修改】400 → 250 (降低门槛，更容易触发)
        self.valid_signal_count = 0  # 连续有效信号帧数
        self.required_valid_frames = 2  # 【修改】3 → 2 (减少连续帧要求)
        ...
        try:
            ...
            if len(audio_data) > 512:
                ...
                try:
                    ...
                    # 【修改3】优化 librosa.beat.beat_track 调用参数
                    # 指定 hop_length 为 512（而非默认值），提高检测精度
                    ...
                    features.beat_times = librosa.frames_to_time(
                        beat_frames, sr=self.sample_rate, hop_length=512
                    ).tolist()
