...
def test_cli_runs_scripted_command(capsys) -> None:
    ...
    assert exit_code == 0
    assert "Please confirm:" in captured.out


def test_cli_inspect_supports_json_output(capsys) -> None:
    exit_code = main(["inspect", "--profile", "demo", "--brain", "llm-assisted", "--transport", "sim", "--format", "json"])
    captured = capsys.readouterr()

    assert exit_code == 0
    assert '"profile": "demo"' in captured.out
    assert '"brain_mode": "llm-assisted"' in captured.out
